...
```bash
docker compose exec omniq-go sh -lc "export PATH=/usr/bin:/bin; export GOTOOLCHAIN=auto; cd /workspace/omniq/validation/scenario-27-consume-drain-false/go && QUEUE=validation-s27-go /usr/bin/go run ."
```
