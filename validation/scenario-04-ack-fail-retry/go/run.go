...
func main() {
	queue := getenv("QUEUE", "validation-s04-go")
	...
	badErr := ""
	result, err := client.AckFail(queue, job.JobID, "bad-token", strPtr("boom"), 0)
	if err != nil {
		badErr = err.Error()
	}

	result, err = client.AckFail(queue, job.JobID, job.LeaseToken, strPtr("boom"), 0)
	if err != nil {
		fail(err)
	}
