...
```bash
docker compose exec omniq-go sh -lc "export PATH=/usr/bin:/bin; export GOTOOLCHAIN=auto; cd /workspace/omniq/validation/scenario-26-consume-drain-true/go && QUEUE=validation-s26-go /usr/bin/go run ."
```
